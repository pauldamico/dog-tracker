...
const {hours, submitTime} = props
    ...
    function updateTreatsAM (clickedTime){
        ...
        const checkIfAdded = treatTime.treatsAM.find(time =>time  === clickedTime)
        checkIfAdded ? treatTime.treatsAM.splice(timeIndex, 1)
      : setTreatTime(prev=>({...prev, treatsAM:[...prev.treatsAM, clickedTime]}))       
      submitTime(treatTime)
    }

    function updateTreatsPM (clickedTime){
        const timeIndex = treatTime.treatsPM.indexOf(clickedTime)        
        const checkIfAdded = treatTime.treatsPM.find(time =>time  === clickedTime)
        checkIfAdded ? treatTime.treatsPM.splice(timeIndex, 1)
      : setTreatTime(prev=>({...prev, treatsPM:[...prev.treatsPM, clickedTime]}))       
      submitTime(treatTime)
          }
...
{dayTimes.map(time=><Time {...time} type={'treats'} updateTime={updateTreatsAM} key={time.id }/>)}
</div>
<section>PM</section>
<div className="pm-div">
{dayTimes.map(time=><Time {...time} type={'treats'} updateTime={updateTreatsPM} key={time.id} />)}
</div>
